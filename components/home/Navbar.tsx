  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'border-b border-border bg-background/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] backdrop-blur-xl'
          : ''
      )}
    >
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {links.map(l => (
            <a
              key={l.href}
              href={l.href}
              className="px-3.5 py-2 text-[13px] font-medium text-muted-foreground transition-colors hover:text-foreground"
            >
              {l.label}
            </a>
          ))}

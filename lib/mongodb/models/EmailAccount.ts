...
const EmailAccountSchema = new Schema<IEmailAccount>(
  {
    ...
    settings: {
      ...
      syncInterval: { type: Number, default: 15, min: 5, max: 1440 },
...
EmailAccountSchema.virtual('computedConnectionStatus').get(function () {
  ...
  if (this.provider === 'smtp' || this.provider === 'imap') {
    return this.smtpConfig?.encryptedUsername ||
      this.imapConfig?.encryptedUsername
      ? 'connected'
      : 'not_configured'
  }
...
EmailAccountSchema.methods.encryptCredentials = function (
  data: string
): string {
  const algorithm = 'aes-256-gcm'
  const secretKey =
    process.env.EMAIL_ENCRYPTION_SECRET || process.env.API_KEY_ENCRYPTION_SECRET
...
EmailAccountSchema.methods.decryptCredentials = function (
  encrypted: string
): string {
  const algorithm = 'aes-256-gcm'
  const secretKey =
...
EmailAccountSchema.methods.setSmtpCredentials = function (
  username: string,
  password: string
) {
...
EmailAccountSchema.methods.setImapCredentials = function (
  username: string,
  password: string
) {
...
EmailAccountSchema.methods.getSmtpCredentials = function () {
  if (
    !this.smtpConfig?.encryptedUsername ||
    !this.smtpConfig?.encryptedPassword
  ) {
    return null
  }
...
EmailAccountSchema.methods.getImapCredentials = function () {
  if (
    !this.imapConfig?.encryptedUsername ||
    !this.imapConfig?.encryptedPassword
  ) {
    return null
  }
...
EmailAccountSchema.methods.setOAuthTokens = function (
  accessToken: string,
  refreshToken: string,
  expiresIn: number
) {
...
EmailAccountSchema.statics.findByUser = function (
  userId: string,
  workspaceId: string
) {
  return this.find({
...
EmailAccountSchema.statics.findDefaultAccount = function (
  userId: string,
  workspaceId: string
) {
  return this.findOne({
...
EmailAccountSchema.statics.setAsDefault = async function (
  accountId: string,
  userId: string,
  workspaceId: string
) {
  ...
  return this.findByIdAndUpdate(accountId, { isDefault: true }, { new: true })
}
...
export default (mongoose.models.EmailAccount as IEmailAccountModel) ||
  mongoose.model<IEmailAccount, IEmailAccountModel>(
    'EmailAccount',
    EmailAccountSchema
  )

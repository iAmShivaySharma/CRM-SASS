...
assetAllocationSchema.virtual('daysOverdue').get(function () {
  ...
  return Math.ceil(
    (now.getTime() - this.expectedReturnDate.getTime()) / (1000 * 60 * 60 * 24)
  )
})
...
assetAllocationSchema.methods.returnAsset = function (
  returnedBy: string,
...
assetAllocationSchema.methods.extendAllocation = function (
  newReturnDate: Date
) {
...
assetAllocationSchema.methods.signAgreement = function (
  filename: string,
  url: string
) {
...
assetAllocationSchema.statics.getActiveAllocations = function (
  workspaceId: string
) {
...
assetAllocationSchema.statics.getOverdueAllocations = function (
  workspaceId: string
) {
  return this.find({
    workspaceId,
...
assetAllocationSchema.statics.getEmployeeAllocations = function (
  employeeId: string,
  status?: string
) {
...
assetAllocationSchema.statics.getAllocationStats = function (
  workspaceId: string
) {
...
export const AssetAllocation =
  mongoose.models.AssetAllocation ||
  mongoose.model<IAssetAllocation>('AssetAllocation', assetAllocationSchema)

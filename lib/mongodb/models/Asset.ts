...
assetSchema.virtual('warrantyStatus').get(function () {
  ...
  if (this.warranty.expiryDate > now) {
    const daysUntilExpiry = Math.ceil(
      (this.warranty.expiryDate.getTime() - now.getTime()) /
        (1000 * 60 * 60 * 24)
    )
...
assetSchema.statics.getAvailableAssets = function (
  workspaceId: string,
  category?: string
) {
...
assetSchema.statics.getWarrantyExpiring = function (
  workspaceId: string,
  days: number = 30
) {
...
assetSchema.statics.searchAssets = function (
  workspaceId: string,
  searchTerm: string
) {
  return this.find({

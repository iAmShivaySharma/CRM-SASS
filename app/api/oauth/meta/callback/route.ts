...
import { MetaOAuthProvider } from '@/lib/auth/social-oauth-providers'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error || !code || !stateParam) {
      log.error('Meta callback error', { error, hasCode: !!code })
    ...
    await SocialOAuthConnection.findOneAndUpdate(
      ...
      },
      {
        ...
        isActive: true,
        metadata: { metaId: userInfo.id },
      },
      { upsert: true, new: true }
    )

    log.info('Meta account connected', {
      userId: state.userId,
      workspaceId: state.workspaceId,
      metaId: userInfo.id,
    })
    ...
    log.error('Meta OAuth callback error:', error)
    return popupResponse('OAUTH_ERROR')
  }
}

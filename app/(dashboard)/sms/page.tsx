...
import AutoReplyPanel from '@/components/marketing/AutoReplyPanel'
...
export default function SmsPage() {
  ...
  return (
    <div className="p-6">
              ...
              {(templatesData?.templates ?? []).length === 0 ? (
                ...
                (templatesData?.templates ?? []).map(t => (
                  ...
                  >
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs ${t.isActive ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}
                    >
              ...
              {(logsData?.logs ?? []).length === 0 ? (
                ...
                (logsData?.logs ?? []).slice(0, 10).map(log => (
                  ...
                  >
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs ${log.status === 'delivered' ? 'bg-primary/10 text-primary' : log.status === 'failed' ? 'bg-destructive/10 text-destructive' : 'bg-muted text-muted-foreground'}`}
                    >
                      {log.status}
                    </span>
                  </div>
      ...
      <div className="mt-6">
        <AutoReplyPanel channel="sms" workspaceId={currentWorkspace._id} />
      </div>
    </div>
  )
}
